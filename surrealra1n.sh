...
case "$1" in
    --seprmvr64-ipsw)
        ...
        if [[ $IOS_VERSION == 9.* ]]; then
            ...
            if [[ $ios8ramdisk == y || $ios8ramdisk == Y || $IDENTIFIER == iPod7* || $FORCE_ACTIVATE == 1 ]]; then
                ...
                rdskipsw=$(./bin/zenity --file-selection --title="Select the iOS $ramdiskversion IPSW file" --file-filter="*.ipsw")
    ...
    --seprmvr64-boot)
        ...
        if [[ ! -d "$savedir" ]]; then
            ...
            IPSW_PATH=$(./bin/zenity --file-selection --title="Select the iOS $IOS_VERSION IPSW file" --file-filter="*.ipsw")
            sleep 2
        else
            echo "first, your device needs to be in pwndfu mode. pwning with gaster"
            echo "[!] Linux has low success rate for the checkm8 exploit on A6-A7. If possible, you should connect your device to a Mac or iOS device and pwn with ipwnder"
    ...
    --make-custom-ipsw)
        ...
        if [[ "$IOS_VERSION" == 10.1* || "$IOS_VERSION" == 10.2* ]]; then
            ...
            if [[ $IOS_VERSION == 14.0* || $IOS_VERSION == 14.1* || $IOS_VERSION == 14.2* ]] && [[ $IDENTIFIER == iPhone10* ]]; then
                ...
                IPSW_PATH=$(./bin/zenity --file-selection --title="Select the iOS 14.3 IPSW file (for iBSS and iBEC)" --file-filter="*.ipsw")
...
# deprecate ota downgrade option
    --downgrade)
        ...
        if [[ $vers == 11.3* || $vers == 11.4* || $vers == 12.* || $vers == 13.* || $vers == 14.* || $vers == 15.* || $vers == 16.* ]]; then
           ...
           IPSW_PATH=$(./bin/zenity --file-selection --title="Select the iOS 10.3.3 IPSW file (for SEP firmware)" --file-filter="*.ipsw")
           mkdir tmp
           mkdir tmp/Firmware
           mkdir tmp/Firmware/all_flash
    ...
    --boot)
        ...
        if [[ ! -d "$BOOT_DIR" ]]; then
            ...
            if [[ $IDENTIFIER == iPhone10,3 ]]; then
                IPSW_PATH_2=$(./bin/zenity --file-selection --title="Select the iOS $LATEST_VERSION IPSW file" --file-filter="*.ipsw")
